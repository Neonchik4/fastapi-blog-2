...
class SUserUpdate(BaseModel):
    ...
    phone_number: str | None = Field(
        None, description="Номер телефона в международном формате, начинающийся с '+'"
    )
    email: EmailStr | None = Field(None, description="Электронная почта")
    password: str | None = Field(
        None, min_length=5, max_length=50, description="Новый пароль, от 5 до 50 знаков"
    )
    confirm_password: str | None = Field(
        None, min_length=5, max_length=50, description="Повторите пароль"
    )
    role_id: int | None = Field(None, description="ID роли пользователя")

    @field_validator("phone_number")
    def validate_phone_number(cls, value: str | None) -> str | None:
        if value and not re.match(r"^\+\d{5,15}$", value):
            raise ValueError(
                'Номер телефона должен начинаться с "+" и содержать от 5 до 15 цифр'
            )
        return value
