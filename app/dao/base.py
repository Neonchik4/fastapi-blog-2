...
class BaseDAO(Generic[T]):
    ...
    async def upsert(
        ...
        if not filter_dict:
            raise ValueError(
                "Upsert требует хотя бы одно уникальное поле в values (unique_fields)."
            )
        try:
            # Создание динамической модели фильтра для find_one_or_none
            FilterModel = create_model(
                f"{cls.model.__name__}UpsertFilter",
                **{k: (Optional[type(v)], v) for k, v in filter_dict.items()},
            )
            existing = await cls.find_one_or_none(session, FilterModel(**filter_dict))
            if existing:
