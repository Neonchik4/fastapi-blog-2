...
def test_load_likes_branches(likes_json_file: Path):
    ...
    # list with non-dicts => filter them out
...
async def test_compute_stats_with_data_and_likes(
    ...
    await set_user_role(u2.id, 3)  # Admin
    ...
    # likes.json: valid likes + junk + non-existent post
    likes_json_file.write_text(
        json.dumps(
            [
                {"user_id": u1.id, "post_id": 999999, "liked": True},  # no such blog
                {"user_id": u1.id, "post_id": 123, "liked": False},  # ignored
                {"user_id": str(u2.id), "post_id": b1.id, "liked": True},  # bad types
    ...
    # roles breakdown should include at least User and Admin
    ...
    # avg tags per blog considers only blogs that have tags -> b1 has 2 tags
    assert stats["avg_tags_per_blog"] == 2.0

    # likes: only entries liked True with int user_id/post_id
    # В likes_total считаются все валидные лайки (liked=True, int user_id, int post_id)
    # независимо от того, существует ли пост в БД
    # u1->999999 (не существует, но валидный), u1->b1, u3->b1, u3->b2 = 4 лайка
    assert stats["likes_total"] == 4
    assert stats["unique_likers"] == 2  # u1 and u3

    # top posts by likes: includes only existing blogs
